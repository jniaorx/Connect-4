...
from simulator import GameController
import game_utils
...
class RandomAgent:
    ...
    def make_move(self, state):
        ...
        game = ConnectFour()
        game.state = state.copy()
        valid_moves = game.get_valid_col_id()
        return random.choice(valid_moves)

def simulate_games(ai_agent, num_games=100):
    ai_wins = 0
    random_wins = 0
    draws = 0
    ...
    for _ in range(num_games):
        ...
        agents = [random_agent, ai_agent] if ai_agent.player_id == 2 else [ai_agent, random_agent]
        game = GameController(board=board, agents=agents)

        result = game.run()

        if result == ai_agent.player_id:
            ai_wins += 1
        elif result == random_agent.player_id:
            random_wins += 1
        else:
            draws += 1
...
if __name__ == "__main__":
    ai_agent = AIAgent(2)
    simulate_games(ai_agent, num_games=100)